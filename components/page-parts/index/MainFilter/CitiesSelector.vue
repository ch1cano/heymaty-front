<template>
  <div class="select selcted-none">
    <v-select
      ...
      :options="options"
      v-model="selected"
      :placeholder="$t('index.citiesPlaceholder')"
      :closeOnSelect="false"
    ...
    >
      ...
      <template #no-options="{}"> {{ $t("index.citiesNoMatch") }}</template>
    </v-select>
  </div>
</template>
<script>
...
export default {
  ...
  computed: {
    ...
    selected: {
      ...
      set: function(val) {
        this.SET_SELECTED({ field: "selectedCities", value: val });
      },
    },
  },
  methods: {
