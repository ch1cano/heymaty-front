  ...
  <header :class="{ inner: $route.path === '/settings' }" class="header">
    <div class="wrap flex">
      ...
      <ul ref="mobileMenu" :class="{ opened: isMenu }" :style="isMenu ? `overflow:unset` : `overflow:hidden`">
        ...
        <li>
          <div v-if="!auth" class="buttons">
            <button @click.prevent="changeModal('signUp', true)">{{ $t("header.register") }}</button>
            <button @click.prevent="changeModal('signIn', true)">{{ $t("header.login") }}</button>
          </div>
          <div v-else class="links">
            <a v-for="(item, i) in menu" :key="i" :style="item.showFor.includes(user.role) ? `` : `display:none`"
              @click.prevent="goTo(localePath(item.link)), (isMenu = false)" :href="item.link" />
            <a class="clearfix" @click.prevent="toggleDropdown()" href="#">
              <transition name="slide-bottom" mode="out-in">
                <div v-if="isDropdown" class="header-profile">
                  <div @click="goTo(localePath('/Settings/Personal'))" class="header-profile__item">{{
                    $t("header.settings") }}</div>
                  <div @click="goTo(localePath(`/profile/${getLink(user)}`))" class="header-profile__item">{{
                    $t("header.myprofile") }}</div>
                  <div @click="logOut()" class="header-profile__item">{{ $t("header.logout") }}</div>
                </div>
              </transition>
            </a>
          </div>
      ...
      <div v-else class="links">
        ...
        <a @click.prevent="toggleDropdown()" href="#" v-click-outside="toggleDropdown">
          <transition name="slide-bottom" mode="out-in">
            <div v-if="isDropdown" class="header-profile">
              <div @click="goTo(localePath(`/profile/${getLink(user)}`))" class="header-profile__item">{{
                $t("header.myprofile") }}</div>
              <div @click="goTo(localePath(`/dialoges`))" class="header-profile__item"> Chats </div>
              <div @click="goTo(localePath(`/favorites`))" class="header-profile__item"> Favorites </div>
              <div @click="goTo(localePath(`/Privacy`))" class="header-profile__item"> Privacy Policy </div>
              <div @click="goTo(localePath(`/Terms`))" class="header-profile__item"> Terms of Service </div>
              <div @click="goTo(localePath('/Settings/Personal'))" class="header-profile__item">{{ $t("header.settings")
              }}</div>
              <div @click="logOut()" class="header-profile__item">{{ $t("header.logout") }}</div>
            </div>
          </transition>
